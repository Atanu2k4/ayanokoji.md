...
`baseline` (no skill) · `ayanokoji` · `caveman` · `yagni` ("Follow YAGNI principles.") ·
...
instruction matches ayanokoji, the benchmark should show it.
...
- **src_loc / src_files**: over-engineering proxy. **Tests are excluded** and tracked separately
  (`wrote_tests_rate`), since writing a test is the discipline ayanokoji prescribes, not bloat.
...
python run.py --task tmpl-fe-datepicker,tmpl-fe-colorpicker,tmpl-fe-command,tmpl-fe-dropzone,tmpl-fe-wizard,tmpl-fe-rating,tmpl-be-duplicate,tmpl-be-search,tmpl-be-count,tmpl-be-archive,tmpl-be-bulkdelete,tmpl-be-csv \
  --arms baseline,caveman,ayanokoji,yagni-oneliner --models haiku --runs 4 --workers 6
...
- **12 real-repo features** (LOC via `git diff`): ayanokoji cuts **60–94%** on features with an
...
- **6 surgical safety tasks** (produced code executed against adversarial input): baseline,
  caveman, and ayanokoji are **100% safe** (20/20); `yagni-oneliner` is **95%** (19/20), it dropped
...
> baseline was contaminated by the ayanokoji plugin's `SessionStart` hook firing on every arm, so
> the baseline was secretly running ayanokoji. Isolation is now enforced with `--setting-sources
