...
- Across **12 classic edge-case traps** (off-by-one, n=0, leap-century, subtractive Roman,
  deep nesting, …) on **two weak models** (`gpt-4.1-mini`, `gpt-5.4-mini`), Ayanokoji holds
...
- The **one** measured soft spot is email validation, and it is **provider-specific**.
  ...
  ayanokoji's target platform, email is **100%** (haiku/sonnet/opus, n=40 each).
- The slip is **not fixable by skill text**: 8 distinct SKILL.md edits (including an n=100
  ...
  skill text that doesn't move the number is exactly the cargo-cult Ayanokoji exists to avoid.

## Method

`baseline` (no skill) vs `ayanokoji` (full SKILL.md), single-shot, default params,
...
All 12 algorithmic tasks: `baseline 20/20 == ayanokoji 20/20` on **both** models. Examples
...
The one place ayanokoji measurably affects correctness is **email validation**, via the
...
**OpenAI (email, baseline vs ayanokoji, n=50–100):**

| model | baseline | ayanokoji |
|---|--:|--:|
| gpt-4.1-mini | 100% | 98% |
| gpt-4.1 | 100% | 79% |
| gpt-5.4-mini | ~100% | ~92% |
...
**Claude (email, baseline vs ayanokoji, n=40):**
...
narrow: ayanokoji writes the plain correct bool the task implies, while the unconstrained
model over-builds the interface and trips a naive `if validate(x)` caller. `url`,
`creditcard`, and `ipv4` hold at ~100% under ayanokoji on both providers, because their lazy
...
shipped. Adding skill text that doesn't work is the cargo-cult Ayanokoji exists to prevent.

## Conclusion

"Ayanokoji degrades model performance" is not supported. Across 12 edge-case traps, ayanokoji
