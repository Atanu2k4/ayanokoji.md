...
- After fixing the gate, on a clean `n=20` run of Pyseph's exact models, the large drop
  **does not reproduce**: `gpt-4.1-mini` is 100% with *and* without Ayanokoji.
- Ayanokoji roughly **halves** median code size, the original headline claim, with no
  meaningful correctness cost on instruction-following models.
- One genuine, small Ayanokoji defect surfaced and is reported honestly below.
...
1. **Unfenced code was scored as "no code blocks."** `extractBlocks()` only matched
   ...
   Ayanokoji's terse style, and frequent on `gpt-5.4-mini`) scored an automatic fail even
...
Two arms (baseline = no skill, ayanokoji), Pyseph's two models, the five repo tasks, `n=20`
...
| model | baseline | ayanokoji | median LOC (base → pony) |
|---|--:|--:|--:|
...
On instruction-following models Ayanokoji ties or slightly *beats* baseline. The low
...
always truthy and accepts every address — a real bug Ayanokoji's `return bool(...)` avoids.

## The one real Ayanokoji defect

On `gpt-5.4-mini`, 2 of 20 Ayanokoji email runs failed because the model reached for the
...
The "Ayanokoji hurts correctness" reports trace to a benchmark that could not read terse
