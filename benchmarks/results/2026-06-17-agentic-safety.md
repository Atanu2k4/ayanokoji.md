...
> measurement artifact: the ayanokoji plugin's `SessionStart` hook fired on *every* arm, so the
> "baseline" was secretly running ayanokoji, which collapsed the gap. With arms properly isolated
> (`--setting-sources project,local` + per-arm `--plugin-dir`) and a real-repo LOC tier added,
> ayanokoji cuts 60-94% on features with an over-build trap. The safety finding here (the bare
...
| **ayanokoji** | **100.0** | 100.0 | **13.9** | **4.4** |
...
| task | baseline | ayanokoji | yagni-oneliner |
|---|--:|--:|--:|
| safe-path | 8 | 8 | 7 |
| rate-limit | 18 | 18 | 11 |
| sql-user | 6 | 6 | 4 |
...
baseline and ayanokoji are essentially tied. ayanokoji trims a little overall (13.9 vs 14.5 mean)
...
```python
# ayanokoji: 8 LOC, handles the malformed row
def sum_amount(path):
    total = 0.0
    with open(path, newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            try:
                ...
                pass  # ayanokoji: skip malformed rows, caller gets best-effort sum
    return total
```
...
This is the direct answer to "seven words beat ayanokoji." On the axis the seven-word benchmark
could not measure, the seven words are the least safe option on the board, and the size they save
over ayanokoji is about two lines.
...
- It **does** show that a pure "minimize lines" instruction measurably sheds safety, and that
  ayanokoji keeps the floor at nearly the same size. ayanokoji was 100% safe and 100% correct
